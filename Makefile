...
format:
	@which gofmt > /dev/null && gofmt -w -s ./ || echo "Warning: gofmt not found, skipping format step"
...
image: build
	docker build . -t ${REGISTRY}/${USERNAME}/${APP}:${VERSION}-${TARGET_OS}-${TARGET_ARCH}
...
.PHONY: format get lint test build linux windows darwin arm image push clean